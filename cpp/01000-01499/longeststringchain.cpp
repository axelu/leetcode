...
// again daily challenge 9/23/2023 with new test cases, of which the last one failed, hence changed implementation
...
private:    
    ...
    bool isPredecessor(string& a, string& b) {
        ...
        int na = a.size(), nb = b.size(), j;
        
        if( na + 1 != nb )
            return false;
        
        for(int i = 0; i < nb; ++i) { // index in b
    ...
    int dfs(map<int,vector<string>>::reverse_iterator it, 
        ...
        for(int i = 0; i < it->second.size(); ++i) {
            ...
            if( isPredecessor(word,word2) ) {
                // cout << word << " is predecessor of " << word2 << endl;
                ret = 1;
                if( next(it) != ite )
