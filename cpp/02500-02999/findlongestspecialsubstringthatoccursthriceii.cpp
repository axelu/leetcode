    ...
    int maximumLength(string& s) {
        ...
        for(int i = 1; i < n; ++i) {
            if( s[i] != s[i-1] ) {
                ...
                if( freq[s[i-1]-'a'].size() > 3 ) {
                    sort(freq[s[i-1]-'a'].begin(),freq[s[i-1]-'a'].end(),greater<int>());
                    freq[s[i-1]-'a'].pop_back();
                }
                cnt = 1;
            } else {
                ++cnt;
        ...
        if( freq[s[n-1]-'a'].size() > 3 ) {
            sort(freq[s[n-1]-'a'].begin(),freq[s[n-1]-'a'].end(),greater<int>());
            freq[s[n-1]-'a'].pop_back();
        }
